    ...
    <main className="relative overflow-hidden">
      ...
      <section className="w-full pt-14 md:pt-14 pb-12 md:pb-24 px-8 md:px-20 xl:px-40 bg-gradient-to-b from-[#18121f] to-[#0f0c13] relative">
        ...
        <div className="relative max-w-6xl mx-auto text-center">
          <h1 className="[font-family:'Bahnschrift-Regular',Helvetica] text-6xl md:text-7xl leading-[1.05] mb-10">{t.title}</h1>
      ...
      <section className="w-full px-8 md:px-20 xl:px-40 py-12 md:py-24 relative">
        ...
        <div className="relative max-w-7xl mx-auto">
          <h2 className="[font-family:'Bahnschrift-Regular',Helvetica] text-5xl md:text-6xl mb-14">{t.pillarsTitle}</h2>
      ...
      <section className="w-full px-8 md:px-20 xl:px-40 py-12 md:py-24 bg-[#141018] relative overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto">
          <h2 className="[font-family:'Bahnschrift-Regular',Helvetica] text-5xl md:text-6xl mb-10">{t.howWeWorkTitle}</h2>
          <p className="[font-family:'Inter-Regular',Helvetica] fg-secondary text-lg md:text-xl leading-relaxed max-w-4xl mb-12">
            {t.howWeWorkSubtitle}
          </p>
          <div className="grid md:grid-cols-3 gap-10">
            {t.howWeWorkSteps?.map((s: any, idx: number) => (
              <div key={`${idx}-${s.title}`} className="bg-[#2a2332] border border-[#3d2a5a] rounded-2xl p-6">
                <h3 className="text-xl mb-3 [font-family:'Bahnschrift-Regular',Helvetica]">{s.title}</h3>
                <p className="text-sm fg-secondary leading-relaxed">{s.text}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="relative max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-start gap-14">
            <div className="md:w-1/2">
              <h2 className="[font-family:'Bahnschrift-Regular',Helvetica] text-5xl md:text-6xl mb-8">{t.safetyTitle}</h2>
              <p className="[font-family:'Inter-Regular',Helvetica] fg-secondary text-lg leading-relaxed mb-6">
                {t.safetySubtitle}
              </p>
              <Link href="/policy">
                <Button className="h-[54px] w-[200px] button-type-1 rounded-xl" onClick={()=>{}} text={t.policyButton} />
              </Link>
            </div>
            <div className="grid sm:grid-cols-2 gap-6 flex-1">
              {t.safetyItems?.map((item: any) => (
      ...
      <section className='w-full px-8 md:px-20 xl:px-40 py-12 md:py-24 bg-gradient-to-b from-[#18121f] to-[#0f0c13] relative'>
        <div className="relative max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-start gap-14">
              <div className="md:w-1/2">
                <h2 className="[font-family:'Bahnschrift-Regular',Helvetica] text-5xl md:text-6xl mb-8">{t.developersTitle}</h2>
                <p className="[font-family:'Inter-Regular',Helvetica] fg-secondary text-lg leading-relaxed mb-6">
                  {t.developersSubtitle}
                </p>
              </div>

              <div className="flex flex-wrap gap-6 flex-1">
                {t.developers?.map((d: any, idx: number) => (
                  <div
                    key={`${idx}-${d.title}`}
                  ...
                  >
                    <h3 className="text-lg mb-2 [font-family:'Bahnschrift-Regular',Helvetica]">{d.title}</h3>
                    <p className="text-xs fg-secondary leading-relaxed">{d.text}</p>
                  </div>
                ))}
              </div>
            </div>
