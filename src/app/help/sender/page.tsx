...
export class HelpPage extends Component<HelpPageProps, HelpPageState> {
    ...
    filterSections(): FaqSection[] {
        ...
        if (!search.trim()) {
            // Default view: only Sender section
            const sender = t.faq.sender as { title: string; items: FaqItem[] };
            return [{ title: sender.title, items: sender.items }];
        }
        ...
        return (
            <div className='py-10 px-8 md:px-10 lg:px-[190px] flex flex-col pt-20 md:pt-40 '>
                <div className='flex-1 flex flex-col w-full gap-6 items-center'>
                    <h1 className='text-[40px] md:text-5xl lg:text-7xl'>{t.title} — {t.faq.sender.title}</h1>
