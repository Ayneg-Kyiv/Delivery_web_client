...
const MyOrders: React.FC<MyOrdersProps> = ({ id }) => {
    ...
    const getOrderStatus = (order: DeliveryOrder) => {
        if (order.isDeclined) return t.profile.myOrders.status.declined;
        if (order.isDelivered) return t.profile.myOrders.status.delivered;
        if (order.isPickedUp) return t.profile.myOrders.status.inTransit;
        if (order.isAccepted) return t.profile.myOrders.status.confirmed;
        return t.profile.myOrders.status.awaitingConfirmation;
    };
