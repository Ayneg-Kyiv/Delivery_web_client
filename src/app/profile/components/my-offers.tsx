...
const MyOffers: React.FC<MyReviewsProps> = ({ id }) => {
    ...
    const getOfferStatus = (offer: DeliveryOffer) => {
        if (offer.isDeclined) return t.profile.myOffers.statusValues.declined;
        if (offer.isAccepted) return t.profile.myOffers.statusValues.accepted;
        return t.profile.myOffers.statusValues.awaitingConfirmation;
    };
                        ...
                        <div key={offer.id} className="bg-[#2d1857] rounded-xl flex flex-col md:flex-row items-start p-6 shadow-lg gap-6">
                            ...
                            <div className="flex flex-col  md:w-2/3 w-full bg-[#1a093a] rounded-lg p-4 border border-[#7c3aed]">
                                ...
                                <div className="flex gap-2 items-center text-white mt-2">
                                    <span>{t.profile.myOffers.sender}: {offer.deliveryRequest.senderName}</span>
                                ...
                                <div className="flex gap-2 items-center text-white mt-2">
                                    ...
                                    <span>{t.profile.myOffers.weight}: {offer.deliveryRequest.objectWeight} kg</span>
                                </div>
                            ...
                            <div className="flex flex-col md:w-1/3 w-full bg-[#7c3aed] rounded-lg p-4 border border-[#1a093a] items-end">
                                ...
                                <div className="text-white font-bold mb-2">
                                    {t.profile.myOffers.status}: {getOfferStatus(offer)}
                                </div>
